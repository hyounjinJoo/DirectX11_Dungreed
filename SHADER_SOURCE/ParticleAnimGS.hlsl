...
void main(point VSOut input[1], inout TriangleStream<GSOutput> output)
{
    ...
    if (0 == particleBuffer[input[0].iInstance].defaultParticleInfo.active)
        return;
	
    float3 vWorldPos = input[0].Pos.xyz + particleBuffer[input[0].iInstance].defaultParticleInfo.position.xyz;
