...
namespace hj
{
	Stage1StartTRB::Stage1StartTRB()
	{
		std::shared_ptr<Map> mapData = ResourceManager::Find<Map>(WIDE("MAP_01_S_TRB"));
		if (mapData)
		{
...
#define MapB mMapLTRBlimit.w
	...
	void Stage1StartTRB::Initialize()
	{
		RoomBase::Initialize();
	}
	void Stage1StartTRB::Update()
	{
		RoomBase::Update();
	}
	void Stage1StartTRB::FixedUpdate()
	{
		RoomBase::FixedUpdate();
	}
	void Stage1StartTRB::Render()
	{
		RoomBase::Render();
	}
}