...
namespace hj
{
	Stage1StartLB::Stage1StartLB()
	{
		std::shared_ptr<Map> mapData = ResourceManager::Find<Map>(WIDE("MAP_01_S_LB"));
		if (mapData)
		{
...
#define MapB mMapLTRBlimit.w
			...
			notPass->SetScale(Vector3(tileSize * 27, tileSize * 3.f, 1.f));
	...
	void Stage1StartLB::Initialize()
	{
		RoomBase::Initialize();
	}
	void Stage1StartLB::Update()
	{
		RoomBase::Update();
	}
	void Stage1StartLB::FixedUpdate()
	{
		RoomBase::FixedUpdate();
	}
	void Stage1StartLB::Render()
	{
		RoomBase::Render();
	}
}