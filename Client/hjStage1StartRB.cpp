...
namespace hj
{
	Stage1StartRB::Stage1StartRB()
	{
		std::shared_ptr<Map> mapData = ResourceManager::Find<Map>(WIDE("MAP_01_S_RB"));
		if (mapData)
		{
...
#define MapB mMapLTRBlimit.w
	...
	void Stage1StartRB::Initialize()
	{
		RoomBase::Initialize();
	}
	void Stage1StartRB::Update()
	{
		RoomBase::Update();
	}
	void Stage1StartRB::FixedUpdate()
	{
		RoomBase::FixedUpdate();
	}
	void Stage1StartRB::Render()
	{
		RoomBase::Render();
	}
}