...
namespace hj
{
	Stage1StartTR::Stage1StartTR()
	{
		std::shared_ptr<Map> mapData = ResourceManager::Find<Map>(WIDE("MAP_01_S_TR"));
		if (mapData)
		{
...
#define MapB mMapLTRBlimit.w
			...
			notPass = object::Instantiate<RoomNotPass>(eLayerType::ForeGround
				, Vector3(MapR + (tileSize * -1.f), tileSize * 1.5f, 0.f));
	...
	void Stage1StartTR::Initialize()
	{
		RoomBase::Initialize();
	}
	void Stage1StartTR::Update()
	{
		RoomBase::Update();
	}
	void Stage1StartTR::FixedUpdate()
	{
		RoomBase::FixedUpdate();
	}
	void Stage1StartTR::Render()
	{
		RoomBase::Render();
	}
}