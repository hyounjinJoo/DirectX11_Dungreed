#include "hjBoss1SwordCollider.h"
...
namespace hj
{
	Boss1SwordCollider::Boss1SwordCollider()
	{
		mScript = AddComponent<Boss1SwordColliderScript>();
	...
	void Boss1SwordCollider::Initialize()
	{
		GameObject::Initialize();
	}

	void Boss1SwordCollider::Update()
	{
		GameObject::Update();
	}

	void Boss1SwordCollider::FixedUpdate()
	{
		GameObject::FixedUpdate();
	}

	void Boss1SwordCollider::Render()
	{
		GameObject::Render();
	}

	void Boss1SwordCollider::SetOwnerObject(Boss1Sword* sword)
	{
...
namespace hj
{
	Boss1SwordColliderScript::Boss1SwordColliderScript()
...
#define TOP_COLLISION_CASE		4
	void Boss1SwordColliderScript::OnCollisionEnter(Collider* collider)
	{
		if (nullptr == mRectCollider || nullptr == collider->GetOwner())
			return;

		Boss1Sword* sword = dynamic_cast<Boss1SwordCollider*>(GetOwner())->GetSword();
		if (nullptr == sword)
			return;
	...
	void Boss1SwordColliderScript::SetCollider(Collider2D* collider)
	{
		if (nullptr == collider)
			return;
