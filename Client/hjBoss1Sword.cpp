...
namespace hj
{
		...
		, mSpawnPosXY(Vector2::Zero)
	{
		...
		// 충돌체 생성
		mSwordMoveCollider = object::Instantiate<Boss1SwordCollider>(eLayerType::MonsterAttack);
		...
		Transform* moveCollider = mSwordMoveCollider->GetTransform();
		Vector2 moveColliderScale = GetScaleXY();
		moveColliderScale.x *= 0.5f;
		moveColliderScale.y *= 0.2f;
		moveCollider->SetScaleXY(moveColliderScale);
		moveCollider->SetPositionY(GetScaleY() * 0.1f);
		moveCollider->SetParent(this->GetTransform()); 
