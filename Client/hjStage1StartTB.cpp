...
namespace hj
{
	Stage1StartTB::Stage1StartTB()
	{
		std::shared_ptr<Map> mapData = ResourceManager::Find<Map>(WIDE("MAP_01_S_TB"));
		if (mapData)
		{
...
#define MapB mMapLTRBlimit.w
	...
	void Stage1StartTB::Initialize()
	{
		RoomBase::Initialize();
	}
	void Stage1StartTB::Update()
	{
		RoomBase::Update();
	}
	void Stage1StartTB::FixedUpdate()
	{
		RoomBase::FixedUpdate();
	}
	void Stage1StartTB::Render()
	{
		RoomBase::Render();
	}
}