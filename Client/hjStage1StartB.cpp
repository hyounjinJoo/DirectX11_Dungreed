...
namespace hj
{
	Stage1StartB::Stage1StartB()
	{
		std::shared_ptr<Map> mapData = ResourceManager::Find<Map>(WIDE("MAP_01_S_B"));
		if (mapData)
		{
...
#define MapB mMapLTRBlimit.w
			...
			notPass = object::Instantiate<RoomNotPass>(eLayerType::ForeGround
				, Vector3(MapR - (tileSize * 1.f), 0.f, 0.f));
			notPass->SetScale(Vector3(tileSize * 2.f, tileSize * 12.f, 1.f));
			mGameObjects.push_back(notPass);

			notPass = object::Instantiate<RoomNotPass>(eLayerType::ForeGround
				, Vector3(MapR - (tileSize * 2.5f), MapB + (tileSize * 1.5f), 0.f));
			notPass->SetScale(Vector3(tileSize * 5.f, tileSize * 3.f, 1.f));
			mGameObjects.push_back(notPass);

			notPass = object::Instantiate<RoomNotPass>(eLayerType::ForeGround
				, Vector3(MapL + (tileSize * 5.5f), MapB + (tileSize * 1.5f), 0.f));
			notPass->SetScale(Vector3(tileSize * 11.f, tileSize * 3.f, 1.f));
			mGameObjects.push_back(notPass);
			...
			RoomPassThrough* passThrough = object::Instantiate<RoomPassThrough>(eLayerType::ForeGround
				, Vector3(tileSize * 3.f, MapB + (tileSize * 3.f) - (platformSize * 0.5f), 0.f));
	...
	void Stage1StartB::Initialize()
	{
		RoomBase::Initialize();
	}
	void Stage1StartB::Update()
	{
		RoomBase::Update();
	}
	void Stage1StartB::FixedUpdate()
	{
		RoomBase::FixedUpdate();
	}
	void Stage1StartB::Render()
	{
		RoomBase::Render();
	}
}